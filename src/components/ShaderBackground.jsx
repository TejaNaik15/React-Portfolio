...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[2]
    + mat4(vec4(-6.545563, 1.7946124, -13.604192, -3.8060522), vec4(6.6583457, 31.911177, 25.164474, 91.81147), vec4(11.840538, 4.1503043, -0.7314397, 6.768467), vec4(-6.3967767, 4.034772, 6.1714606, -0.32874924))
    * buf[3]
    ...
    * buf[7]
    ...
    buf[0] = sigmoid(buf[0]);
    vec3 col = vec3(buf[0].x , buf[0].y , buf[0].z);
  ...
  void main() {
    ...
    vec4 c = cppn_fn(uv, 0.1 * sin(0.3 * iTime), 0.1 * sin(0.69 * iTime), 0.1 * sin(0.44 * iTime));
    gl_FragColor = c;
  }
`;

const CPPNShaderMaterial = shaderMaterial(
  { iTime: 0, iResolution: new THREE.Vector2(1, 1), uTint: new THREE.Color('#0b1220') },
  vertexShader,
  fragmentShader
);
...
export default function ShaderBackground({ className = '', tintDark = '#0b1220', tintLight = '#e6eefc' }) {
  const { theme } = useTheme();
  const tint = theme === 'light' ? tintLight : tintDark;
  ...
  React.useEffect(() => {
    ...
    gsap.set(containerRef.current, { filter: 'blur(10px)', scale: 1.04, autoAlpha: 0.85 });
    gsap.to(containerRef.current, { filter: 'blur(0px)', scale: 1, autoAlpha: 1, duration: 1.2, ease: 'power3.out' });
  }, [theme]);
  ...
  return (
    <div ref={containerRef} className={`absolute inset-0 z-0 w-full h-full ${className}`} aria-hidden>
      ...
      <div className={`pointer-events-none absolute inset-0 ${theme === 'light' ? 'bg-white/30' : 'bg-black/20'}`} />
      <div className={`pointer-events-none absolute inset-0 bg-gradient-to-t ${theme === 'light' ? 'from-white/60 via-transparent to-white/30' : 'from-black/40 via-transparent to-black/20'}`} />
    </div>
  );
}

// Drei typings shim for R3F intrinsic element
